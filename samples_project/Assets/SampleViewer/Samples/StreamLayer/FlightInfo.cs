...
public class FlightInfo : MonoBehaviour
{
    ...
    public void SetInfo(string Info)
    {
        ...
        var StadiumMaterials = flightRender.materials;
        if (Info == "National")
        {
    ...
    private void SetOnGround()
    {
        ...
        if (Distance < RayCastDistanceThreshold)
        {
            if (Physics.Raycast(transform.position, Vector3.down, out RaycastHit hitInfo, (float)SpawnHeight))
            {
                ...
                var flightLocationComponent = transform.GetComponent<ArcGISLocationComponent>();
                double NewHeight = flightLocationComponent.Position.Z - hitInfo.distance;
                double flightLongitude = flightLocationComponent.Position.X;
                double flightLatitude = flightLocationComponent.Position.Y;
                ArcGISPoint Position = new ArcGISPoint(flightLongitude, flightLatitude, NewHeight, flightLocationComponent.Position.SpatialReference);
                flightLocationComponent.Position = Position;

                OnGround = true;

                // The features were not being rendered until they are placed on the ground
                flightRender.transform.parent.gameObject.SetActive(true);
            }
        }
    }
